...
GPIOButtons.prototype.listener = debounce(function(action,err,value){
	var self = this;
	...
	// Jan Starzak (2020.05.02): using Pull Up resistor, so value for pressed is 0
	// Jan Starzak (2020.06.22): switched to "onkeyup" (so value checked is 1), to allow for press&hold to eject
	if(value !== lastvalue && value === 1){
		//do thing
		if (!self.ignoreNextUp) {
			self[action]();
			clearTimeout(self.ejectTimeout);
		}
		self.ignoreNextUp = false;
	} else if (value !== lastvalue) {
		if (action === "playPause") {
			self.ejectTimeout = setTimeout(function () {
				self.ignoreNextUp = true;
				self.eject();
			}, 3000);
		}
	}
...
GPIOButtons.prototype.eject = function () {
	//this.logger.info('GPIO-Buttons: Eject button pressed\n');
